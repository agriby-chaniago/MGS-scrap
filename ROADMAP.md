...
Repo baru sekarang murni library: `packages/modelgate-core`, `specs/`, `conformance/` (fixtures + runner, tanpa server/action client), plus dokumen tingkat atas (README, ABOUT, ARCHITECTURE, BACKLOG, ROADMAP, CONTRIBUTING). `docs/uas-archive/` sempat dipertahankan dengan alasan "bukan bagian package yang diarsipkan", tapi ini juga salah — materi kuliah (PRD, slides, video script) tidak relevan untuk repo yang seharusnya murni library. Dihapus di commit susulan (bukan amend lagi — Zenodo sudah punya archival job berjalan untuk `v1.0.0` saat itu, memindah tag berisiko mengganggu proses yang sedang jalan).
